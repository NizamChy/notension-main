...
const VisitedDoctorProfile = () => {
  ...
  const handleAddToFavorite = (event) => {
    ...
    addToFavouriteList(visitedDoctor?.doctorInfo, merchantType);
  };
  ...
  const visitedDoctor =
    centerId !== "null"
      ? profileInfo?.find(
          (profile) => profile?.consultationCenterInfo?._id === centerId
        )
      : profileInfo[0];
  ...
  useEffect(() => {
    if (visitedDoctor) {
      isExists = isAddedToFavouriteList(
        visitedDoctor?.doctorInfo?._id,
        merchantType
      );
      setIsFavoriteAdded(isExists);
    }
  }, [visitedDoctor, handleAddToFavorite]);

  if (progressing) return <Loader />;
  if (!visitedDoctor) return null;
          ...
          <div className="relative md:flex items-center lg:gap-20 text-deepGray lg:border border-slate-200 py-20 rounded-lg">
            <div className="lg:flex items-center md:w-1/2">
              <div className="md:w-1/3">
                <Image
                  src={
                    visitedDoctor?.doctorInfo?.gender === "Female"
            ...
            <div className="md:w-1/2 mt-3 md:mt-0 md:border-0 md:p-0 border border-primary rounded-lg p-3">
              <div>
                ...
                <p className="py-1 text-lg md:text-xl lg:text-2xl font-semibold text-green-600">
                  {visitedDoctor?.consultationCenterInfo?.center_name}
                </p>
                <p className="flex items-start gap-2 pt-1">
                  ...
                  <span className="text-sm md:text-base lg:text-lg font-medium pe-5">
                    {visitedDoctor?.consultationCenterInfo?.address}
                  </span>
                </p>

                {visitedDoctor?.chamber_onDay_time_slot_1 && (
                ...
                {[1, 2, 3].map((index) => {
                  const timeSlot =
                    visitedDoctor?.[`chamber_onDay_time_slot_${index}`];
                  return (
                    timeSlot && (
                      <p
                        key={index}
                ...
                {visitedDoctor?.book_an_appointment &&
                  !visitedDoctor?.is_chamber_off && (
                    <button
                ...
                {visitedDoctor?.consultationCenterInfo
...
export default VisitedDoctorProfile;
